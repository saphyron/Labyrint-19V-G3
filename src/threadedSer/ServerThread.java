	...
	public void run() {
		...
		while (true) {
			...
			if (listedStrFromClient.size() == 2 && listedStrFromClient.get(0).equalsIgnoreCase("spawn")) {
				...
				switch (direction) {
				case "up":    ServerOld.playerMoved(0,-1,"up",name);    break;
				case "down":  ServerOld.playerMoved(0,+1,"down",name);  break; 
				case "left":  ServerOld.playerMoved(-1,0,"left",name);  break;
				case "right": ServerOld.playerMoved(+1,0,"right",name); break;
				default: break;
				}
